...
class IoULoss(nn.Module):
    ...
    def forward(self, pred, target):
        # Slice centers and sizes
        p_ctr, p_wh = pred[:, :2], pred[:, 2:]
        t_ctr, t_wh = target[:, :2], target[:, 2:]

        # Calculate boundaries for predicted boxes
        p_min = p_ctr - p_wh / 2.0
        p_max = p_ctr + p_wh / 2.0
        
        # Calculate boundaries for ground truth
        t_min = t_ctr - t_wh / 2.0
        t_max = t_ctr + t_wh / 2.0

        # Intersection logic
        top_left = torch.max(p_min, t_min)
        bottom_right = torch.min(p_max, t_max)
        
        wh_inter = (bottom_right - top_left).clamp(min=0)
        area_inter = wh_inter[:, 0] * wh_inter[:, 1]

        # Union logic
        area_p = p_wh[:, 0] * p_wh[:, 1]
        area_t = t_wh[:, 0] * t_wh[:, 1]
        area_union = area_p + area_t - area_inter

        iou_val = area_inter / (area_union + self.eps)
        score = 1.0 - iou_val

        if self.reduction == "mean": return score.mean()
        if self.reduction == "sum": return score.sum()
        return score

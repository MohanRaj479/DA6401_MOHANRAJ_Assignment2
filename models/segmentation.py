...
class VGG11UNet(nn.Module):
    """UNet architecture using VGG11 features as the backbone."""
    def __init__(self, num_classes=3, in_channels=3, dropout_p=0.5):
        super().__init__()
        self.encoder = VGG11Encoder(in_channels)
        
        def build_decoder_node(c_in, c_out):
        ...
        # Upsampling and refinement stages
        self.up5 = nn.ConvTranspose2d(512, 512, 2, stride=2)
        self.dec5 = build_decoder_node(1024, 512)
        
        self.up4 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.dec4 = build_decoder_node(768, 256)
        
        self.up3 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.dec3 = build_decoder_node(384, 128)
        
        self.up2 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.dec2 = build_decoder_node(192, 64)
        
        self.up1 = nn.ConvTranspose2d(64, 64, 2, stride=2)
        self.dec1 = build_decoder_node(128, 64)
        
        self.head = nn.Conv2d(64, num_classes, 1)

    def forward(self, img):
        bottleneck, skips = self.encoder(img, return_features=True)
        
        # Decoding with skip connections
        y = self.dec5(torch.cat([self.up5(bottleneck), skips['pool5_pre']], 1))
        y = self.dec4(torch.cat([self.up4(y), skips['pool4_pre']], 1))
        y = self.dec3(torch.cat([self.up3(y), skips['pool3_pre']], 1))
        y = self.dec2(torch.cat([self.up2(y), skips['pool2_pre']], 1))
        y = self.dec1(torch.cat([self.up1(y), skips['pool1_pre']], 1))
